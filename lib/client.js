window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        var CSS = {
            ...
            toggleField: "dshdmc_toggleField",
            ...
            empty: "dshdmc_empty",
            commandField: "dshdmc_commandField",
            commandInput: "dshdmc_commandInput"
        };
        var css =
            ...
            "." + CSS.commandField + "{flex-direction:column;align-items:stretch;gap:6px;display:flex}" +
            "." + CSS.toggleField + "{flex-direction:column;align-items:flex-start;gap:6px;display:flex}" +
            "." + CSS.commandInput +
            "{box-sizing:border-box;background:var(--dsw-alias-bg-module-platform);height:36px;font:inherit;" +
            "color:var(--dsw-alias-label-primary);border:.5px solid var(--dsw-alias-border-l3);border-radius:10px;" +
            "font-size:14px;line-height:22px;padding:0 12px;flex:1;min-width:0}" +
            "." + CSS.commandInput + ":focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:-1px}" +
            "." + CSS.commandInput + ":disabled{cursor:not-allowed;opacity:.4}";
        ...
        var en = {
            ...
            commandLabel: "Docker executable",
            commandHint: "Path to the `docker` CLI used for profile discovery and the gateway spawn. Leave empty to use the auto-detected default. Press Enter or click away to apply.",
            commandPlaceholder: "/usr/bin/docker  ·  C:\\Program Files\\Docker\\...\\docker.exe",
            reduceLabel: "Reduce log output",
            reduceHint: "Keeps the gateway's progress logs out of the dsh console by capturing them into the plugin's stderr log file. Turn it off to debug the raw gateway output; switching restarts the gateway connection.",
        ...
        var DockerMcpCardController = class {
            ...
            /**
             * Write the executable override (persisted). An empty value DROPS
             * the override (so the key disappears from the user layer and the
             * host falls back to the auto-resolved row config); a non-empty
             * path is used verbatim for both discovery and the gateway spawn.
             * The host re-discovers on the new executable and pushes the
             * refreshed `effectiveCommand` + profile list.
             */
            setCommand(command) {
                if (this.disposed) return Promise.resolve();
                var value = typeof command === "string" ? command.trim() : "";
                var write = value === "" ? this.scope.unset("command") : this.scope.set("command", value);
                return Promise.resolve(write).catch((error) => this.noteError(error));
            }
            /**
             * Flip the "Reduce log output" toggle (persisted `stderrMode`:
             * "log" captures the gateway's console noise into the host's log
             * file, "console" echoes it for debugging). The effective mode is
             * read from the served snapshot: an explicit override wins; an
             * empty one (never toggled) falls back to the row-config
             * `rowStderr` the host serves — exactly what the host resolves, so
             * the switch never lies about the current state. The toggle always
             * writes an explicit value (the row config stops applying once the
             * user has touched it).
             */
            toggleStderr() {
                if (this.disposed) return Promise.resolve();
        ...
        function DockerMcpCard(props) {
            ...
            var effectiveCommand = typeof state.effectiveCommand === "string" ? state.effectiveCommand : "";
            // Local draft so typing does not persist on every keystroke: the
            // override is committed on blur/Enter. `commandSync` tracks the
            // last host-served executable this draft mirrors, so a host push
            // (the auto-detected path landing after boot, or an echo from a
            // sibling instance) re-syncs an idle field but never clobbers an
            // edit in progress.
            var commandState = react.useState(effectiveCommand);
            var commandDraft = commandState[0];
            var setCommandDraft = commandState[1];
            var commandSyncState = react.useState(effectiveCommand);
            var commandSync = commandSyncState[0];
            var setCommandSync = commandSyncState[1];
            var commandFocusedState = react.useState(false);
            var commandFocused = commandFocusedState[0];
            var setCommandFocused = commandFocusedState[1];
            if (effectiveCommand !== commandSync) {
                setCommandSync(effectiveCommand);
                if (!commandFocused) setCommandDraft(effectiveCommand);
            }
            /** Commit the trimmed draft: an empty value drops the override so
             * the host falls back to the auto-resolved default. */
            function commitCommand() {
                var value = typeof commandDraft === "string" ? commandDraft.trim() : "";
                if (value === effectiveCommand) return;
                if (typeof props.setCommand !== "function") return;
                props.setCommand(value);
            }
            if (!state.available) return null;
            var error = state.lastRefreshError || state.actionError || "";
            // The "Reduce log output" switch mirrors what the host actually
            // resolves: the explicit `stderrMode` when set, else the
            // row-config fallback (`rowStderr`). Never a phantom state.
            ...
            var reduceChecked = stderrEffective === "log";
            // Discovered ids feed the selector, but only a HEALTHY, non-empty
            // discovery list is authoritative: `default` is offered only when
            // the store actually reports it — no phantom option. On a
            // discovery error or an empty list the profile store is not
            // trustworthy, so `default` stays as the fallback option (plus the
            // current profile, so a picked id never disappears) and the picker
            // is never empty.
            var discovered = state.profiles.filter((id) => typeof id === "string" && id !== "");
            var authoritative = error === "" && discovered.length > 0;
            var options = (authoritative ? discovered : discovered.concat(["default", state.profile])).filter(
            ...
            var missing = error === "" && discovered.indexOf(state.profile) === -1;
                    ...
                    open
                        ? (0, reactJsx.jsxs)("div", {
                                ...
                                children: [
                                    ...
                                    (0, reactJsx.jsxs)("div", {
                                        className: CSS.commandField,
                                        children: [
                                            (0, reactJsx.jsx)("div", { className: CSS.fieldTitle, children: t("commandLabel") }),
                                            (0, reactJsx.jsx)("input", {
                                                className: CSS.commandInput,
                                                type: "text",
                                                value: commandDraft == null ? "" : commandDraft,
                                                ...
                                                onChange: (event) => {
                                                    ...
                                                    setCommandDraft(typeof next === "string" ? next : "");
                                                },
                                                onFocus: () => {
                                                ...
                                                onKeyDown: (event) => {
                                                    if (event && event.key === "Enter") commitCommand();
                                                }
                                            }),
                                            (0, reactJsx.jsx)("div", { className: CSS.fieldDesc, children: t("commandHint") })
                                        ]
                                    }),
                                    (0, reactJsx.jsxs)("div", {
                                        className: CSS.toggleField,
                                        children: [
                                            (0, reactJsx.jsx)(primitives.Switch, {
                                                ...
                                                disabled: !state.writable,
                                                onChange: () => {
                                                    if (typeof props.toggleStderr === "function") props.toggleStderr();
                                                }
                                            }),
                                            (0, reactJsx.jsx)("div", { className: CSS.fieldDesc, children: t("reduceHint") })
                                        ]
                                    }),
