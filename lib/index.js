...
function apply(ctx, config) {
    ...
    const startBridge = () => {
        if (bridge === void 0) bridge = ctx.plugin(McpClient, bridgeConfig(row, runningProfile, runningCommand));
        return bridge;
    };
    ...
    const switchConnection = (profile, command) => {
        ...
        Promise.resolve(bridge.update(bridgeConfig(row, profile, command))).catch((error) =>
