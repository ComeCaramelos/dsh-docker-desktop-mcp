 ...
 * one executable. Args keep the user's original invocation shape:
...
function apply(ctx, config) {
    ...
    /**
     * Announce where the gateway's console output goes. "log" mode (the
     * default) prints ONE line pointing at the log file instead of echoing
     * hundreds of progress lines through the inherited stderr; an
     * unavailable redirect (no POSIX shell) warns once so the silent
     * fallback is not mysterious.
     */
    const noteGatewayStderr = (built) => {
        if (config.gatewayStderr === "console") return;
        const key = built.fallback === "" ? built.redirect : `fallback:${built.fallback}`;
    ...
    const startBridge = () => {
        if (bridge === void 0) {
            const built = bridgeConfig(row, runningProfile, runningCommand);
    ...
    const switchConnection = (profile, command) => {
        ...
        const built = bridgeConfig(row, profile, command);
        noteGatewayStderr(built);
